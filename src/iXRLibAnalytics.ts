import { InitAllStatics, iXRInitAllStatics } from './iXRLibGlobal';
...
import { iXRAIProxy, iXRBase, iXRDbContext, iXRLog, iXREvent, iXRStorage, iXRTelemetry, iXRLibConfiguration, DbSetStorage } from './iXRLibCoreModel';
			...
			while (!bDoneSending && (dsIXRXXX?.Count() ?? 0) > 0 && ((bSendingStragglers || (dsIXRXXX?.Count() ?? 0) >= nConfiguredXXXPerSendAttempt) || (!iXRLibStorage.m_ixrLibConfiguration.m_bUseDatabase)))
			{
				...
				for (i = 0; i < iXRLibStorage.m_ixrLibConfiguration.m_nSendRetriesOnFailure; i++)
				{
					try
					{
						...
						{
							await Sleep(iXRLibStorage.m_ixrLibConfiguration.m_tsSendRetryInterval.ToInt64() * 1000);
						}
					}
					catch (error)
					{
